  ...
  return (
    <div className={`relative ${width} ${height} ${borderRadius} bg-custom-togglegray cursor-pointer mb-4 flex flex-col justify-center p-4`}>
      {isDetailBox ? (
        <>
          {box.map((item, index) => (
            <div key={index} className="flex items-center justify-between mb-6 m-3"> {/* Increased margin-bottom */}
              <p className="text-left font-bold text-white">{item.name}</p>
              <div className="w-32 h-8 flex items-center justify-center bg-custom-gray text-white rounded-full"> {/* Box for numbers */}
                {isPremiumPlan ? (
                  <span className="font-bold">{item.number}</span>
                ) : (
                  <>
                    <span className="mr-3">-</span> {/* Minus icon */}
                    <span className="font-bold">{item.number}</span>
                    <span className="ml-3">+</span> {/* Plus icon */}
                  </>
                )}
              </div>
            </div>
